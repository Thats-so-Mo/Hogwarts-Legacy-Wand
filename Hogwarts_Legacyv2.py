...
#Thanks to thistallasian (reddit user) for their suggestion in assigning F1-F4 to each row and then having the wand change row and then cast the correct corresponding spell. Removing the need to change rows and then cast the correct spell.
...
class GestureWand(Wand):
    def __init__(self, *args, **kwargs):
        ...
        self.gestures = {
            ...
            ("DR", "R", "UR", "U", "UL"): "reparo",
            ("UR", "R", "DR", "UR", "R", "DR"): "conjuring_spell", #made this movement completely up
            ("DR", "R", "UR"): "depulso",
            ("UR", "DR", "UR", "UR", "D", "DR", "DL", "L"): "glacius",
            ("U", "UL", "L", "DL", "D", "DR", "R"): "descendo",
            ("DR", "R", "UL", "L", "R", "D"): "reparo",
            ("UR", "L", "DR", "L"): "crucio", #made this one up completely          
            ("U", "DR", "DL", "DR"): "revelio",
            ("DR", "UR"): "protego",
            ("UR", "R", "DR"): "accio",
            ("DR", "U", "DL"): "ancient",
...
#            transition = math.ceil(sleep * 10)
            ...
            if wand.spell == "expelliarmus":
                ...
                write_report(NULL_CHAR*2+chr(58)+NULL_CHAR*5) #Changes to first row (F1)
                write_report(NULL_CHAR*8)
            ...
            if wand.spell == "lumos":
                ...
                write_report(NULL_CHAR*2+chr(59)+NULL_CHAR*5) #Changes to second row (F2)
                write_report(NULL_CHAR*8)                
                write_report(NULL_CHAR*2+chr(30)+NULL_CHAR*5) #press number 1
                write_report(NULL_CHAR*8)
                wand.spell = None
            ...
            if wand.spell == "reparo":
                ...
                write_report(NULL_CHAR*8)
                wand.spell = None
            if wand.spell == "avada_kedavra":
                print('avada_kedavra spell detected')
                write_report(NULL_CHAR*2+chr(59)+NULL_CHAR*5) #Changes to second row (F2)
                write_report(NULL_CHAR*8)                 
                write_report(NULL_CHAR*2+chr(33)+NULL_CHAR*5) #press number 4
                write_report(NULL_CHAR*8)
            ...
            if wand.spell == "locomotor":
                print('locomotor spell detected')
                write_report(NULL_CHAR*2+chr(60)+NULL_CHAR*5) #Changes to third row (F3)
                write_report(NULL_CHAR*8)                 
                write_report(NULL_CHAR*2+chr(30)+NULL_CHAR*5) #press number 1
                write_report(NULL_CHAR*8)
                wand.spell = None
            ...
            if wand.spell == "depulso":
                print('depulso spell detected')
                write_report(NULL_CHAR*2+chr(61)+NULL_CHAR*5) #Changes to fourth row (F4)
                write_report(NULL_CHAR*8)                  
                write_report(NULL_CHAR*2+chr(30)+NULL_CHAR*5) #press number 1
                write_report(NULL_CHAR*8)
                wand.spell = None
            if wand.spell == "glacius":
                print('Glacius spell detected')
                write_report(NULL_CHAR*2+chr(61)+NULL_CHAR*5) #Changes to fourth row (F4)
                write_report(NULL_CHAR*8)                     
                write_report(NULL_CHAR*2+chr(31)+NULL_CHAR*5) #press number 2
                # Release keys
                write_report(NULL_CHAR*8)
                wand.spell = None
            if wand.spell == "descendo":
                print('descendo spell detected')
                write_report(NULL_CHAR*2+chr(61)+NULL_CHAR*5) #Changes to fourth row (F4)
                write_report(NULL_CHAR*8)                     
                write_report(NULL_CHAR*2+chr(32)+NULL_CHAR*5) #press number 3
                # Release keys
                write_report(NULL_CHAR*8)
                wand.spell = None
            if wand.spell == "crucio":
                print('crucio spell detected')
                write_report(NULL_CHAR*2+chr(61)+NULL_CHAR*5) #Changes to fourth row (F4)
                write_report(NULL_CHAR*8)                     
                write_report(NULL_CHAR*2+chr(33)+NULL_CHAR*5) #press number 4
                write_report(NULL_CHAR*8)
